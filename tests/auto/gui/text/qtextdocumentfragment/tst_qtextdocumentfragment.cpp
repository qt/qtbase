...
static constexpr struct QTextHtmlEntity { QLatin1StringView name; char16_t code; } entities[]= {
    { "AElig"_L1, 0x00c6 },
    { "AMP"_L1, 38 },
    { "Aacute"_L1, 0x00c1 },
    { "Acirc"_L1, 0x00c2 },
...
bool operator<(const QTextHtmlEntity &lhs, const QTextHtmlEntity &rhs)
{
...
{
    Q_ASSERT(std::is_sorted(std::begin(entities), std::end(entities)));
    QTest::addColumn<QString>("html");
    QTest::addColumn<char16_t>("code");

    for (const auto &entity : entities) {
        QTest::newRow(entity.name.latin1()) << QString("<pre>&") + QString(entity.name) + QString(";</pre>")
                                            << entity.code;
    }
}
