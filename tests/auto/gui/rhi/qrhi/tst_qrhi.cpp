...
private slots:
    ...
    void indexedIndirectMultiDrawHighDrawCountMultisample();
    void indexedIndirectMultiDrawHighDrawCountMultisampleDepthResolve_data();
    void indexedIndirectMultiDrawHighDrawCountMultisampleDepthResolve();
    void indexedIndirectMultiDrawHighDrawCountRenderBufferDepth_data();
    void indexedIndirectMultiDrawHighDrawCountRenderBufferDepth();
    ...
    void indexedIndirectDrawCountMultisample_data();
    void indexedIndirectDrawCountMultisample();
    void baseInstanceDraw_data();
    void baseInstanceDraw();
    void indirectCommandBufferBaseline_data();
    void indirectCommandBufferBaseline();
    void indirectCommandBufferNonIndexed_data();
    void indirectCommandBufferNonIndexed();
    void indirectCommandBufferSubrange_data();
    void indirectCommandBufferSubrange();
    void indirectCommandBufferFromCompute_data();
    void indirectCommandBufferFromCompute();
    void indirectCommandBufferFromComputeSubrange_data();
    void indirectCommandBufferFromComputeSubrange();
    void indirectCommandBufferFromComputeCount_data();
    void indirectCommandBufferFromComputeCount();
    void indirectCommandBufferRerecord_data();
    void indirectCommandBufferRerecord();
    void indirectCommandBufferCpuThenBuild_data();
    void indirectCommandBufferCpuThenBuild();
...
struct IndirectTestVertex {
    float x, y, z;
    float r, g, b, a;
};

static constexpr IndirectTestVertex IndirectTestVertices[] = {
...
void tst_QRhi::indexedIndirectMultiDrawBaseline()
{
    QFETCH(QRhi::Implementation, impl);
    QFETCH(QRhiInitParams *, initParams);
    ...
    static constexpr QRhiIndexedIndirectDrawCommand IndirectDrawCommands[] = {
        { 6, 1, 0, 0, 0 }, // vertexOffset = 0  -> left quad
    ...
    // Core check: drawCount=2 (we do NOT require DrawIndirectMulti)
...
// Same as indexedIndirectMultiDrawBaseline but with a custom stride between draw commands
void tst_QRhi::indexedIndirectMultiDrawCustomStride()
{
    QFETCH(QRhi::Implementation, impl);
    QFETCH(QRhiInitParams *, initParams);
    ...
    struct PaddedDrawCommand {
        ...
        quint32 pad[3]; // 12 bytes padding -> stride != sizeof(QRhiIndexedIndirectDrawCommand)
    };
    static constexpr PaddedDrawCommand IndirectDrawCommands[] = {
        { { 6, 1, 0, 0, 0 }, { 0xBEEFCAFE, 0xDEADCAFE, 0xF00DCAFE }, }, // vertexOffset = 0  -> left quad
        { { 6, 1, 0, 4, 0 }, { 0xBEEFCAFE, 0xDEADCAFE, 0xF00DCAFE }, }, // vertexOffset = 4  -> right quad
    };
    ...
    // Core check: drawCount=2 (we do NOT require DrawIndirectMulti)
    // The custom stride will trigger CPU emulation on some backends
    cb->drawIndexedIndirect(db.get(), 0, std::size(IndirectDrawCommands), sizeof(PaddedDrawCommand));
...
void tst_QRhi::indexedIndirectMultiDrawFromCompute_data()
{
    rhiTestData();
}

void tst_QRhi::indexedIndirectMultiDrawFromCompute()
{
    QFETCH(QRhi::Implementation, impl);
    QFETCH(QRhiInitParams *, initParams);
    ...
    std::unique_ptr<QRhiBuffer> db(rhi->newBuffer(QRhiBuffer::Static, QRhiBuffer::StorageBuffer | QRhiBuffer::IndirectBuffer, sizeof(EmptyCommands)));
    QVERIFY(db->create());
    u->uploadStaticBuffer(db.get(), EmptyCommands);

    std::unique_ptr<QRhiShaderResourceBindings> computeSrb(rhi->newShaderResourceBindings());
    computeSrb->setBindings({ QRhiShaderResourceBinding::bufferLoadStore(0, QRhiShaderResourceBinding::ComputeStage, db.get()) });
    QVERIFY(computeSrb->create());

    std::unique_ptr<QRhiComputePipeline> computePipeline(rhi->newComputePipeline());
    const QShader cs = loadShader(":/data/indirect_draw_args.comp.qsb");
    QVERIFY(cs.isValid());
    computePipeline->setShaderStage({ QRhiShaderStage::Compute, cs });
    computePipeline->setShaderResourceBindings(computeSrb.get());
    QVERIFY(computePipeline->create());

    cb->beginComputePass(u);
    cb->setComputePipeline(computePipeline.get());
    ...
    cb->drawIndexedIndirect(db.get(), 0, 2);
...
// Shared tail for the QRhiIndirectCommandBuffer tests: checks a horizontal
// line in the middle of the readback against the expected number of red, green
// and blue pixels. White (the "must not be drawn" quad) fails the color check.
static void checkIndirectCommandBufferResult(QRhi *rhi, const QImage &image, const QSize &expectedSize,
                                             int expectedRed, int expectedGreen, int expectedBlue)
{
    ...
    QCOMPARE(redCount, expectedRed);
    QCOMPARE(greenCount, expectedGreen);
    QCOMPARE(blueCount, expectedBlue);
}

void tst_QRhi::indirectCommandBufferBaseline_data()
{
    rhiTestData();
}

// The QRhiIndirectCommandBuffer counterpart of
// indexedIndirectMultiDrawBaseline: the same two quads, but the draw commands
// are recorded on the CPU into an indirect command buffer and replayed with a
// single executeIndirect().
void tst_QRhi::indirectCommandBufferBaseline()
{
    QFETCH(QRhi::Implementation, impl);
    QFETCH(QRhiInitParams *, initParams);
    ...
    std::unique_ptr<QRhiIndirectCommandBuffer> icb(rhi->newIndirectCommandBuffer(QRhiIndirectCommandBuffer::IndexedDraws, 8));
    QVERIFY(icb->create());

    icb->clear();
    icb->drawIndexed(6, 1, 0, 0, 0); // left quad
    icb->drawIndexed(6, 1, 0, 4, 0); // right quad
    QCOMPARE(icb->commandCount(), 2u);
    u->commitIndirectCommandBuffer(icb.get());
    ...
    checkIndirectCommandBufferResult(rhi, result, texture->pixelSize(), 64, 64, 0);
}

void tst_QRhi::indirectCommandBufferNonIndexed_data()
{
    rhiTestData();
}

// Same, with a Draws (non-indexed) indirect command buffer.
void tst_QRhi::indirectCommandBufferNonIndexed()
{
    QFETCH(QRhi::Implementation, impl);
    QFETCH(QRhiInitParams *, initParams);
    ...
    std::unique_ptr<QRhiBuffer> vb(rhi->newBuffer(QRhiBuffer::Immutable, QRhiBuffer::VertexBuffer, sizeof(IndirectTestVerticesNonIndexed)));
    QVERIFY(vb->create());
    ...
    std::unique_ptr<QRhiIndirectCommandBuffer> icb(rhi->newIndirectCommandBuffer(QRhiIndirectCommandBuffer::Draws, 8));
    QVERIFY(icb->create());

    icb->clear();
    icb->draw(6, 1, 0, 0); // left quad
    icb->draw(6, 1, 6, 0); // right quad
    QCOMPARE(icb->commandCount(), 2u);

    // Recording the wrong kind must be rejected, not silently corrupt the batch.
    QTest::ignoreMessage(QtWarningMsg, "QRhiIndirectCommandBuffer: drawIndexed() on a Draws indirect command buffer; ignored");
    icb->drawIndexed(6, 1, 0, 0, 0);
    QCOMPARE(icb->commandCount(), 2u);
...
void tst_QRhi::indirectCommandBufferSubrange_data()
{
    rhiTestData();
}

// Records three commands - left red quad, right green quad, and a full width
// white quad that must not be drawn - then executes only the middle one. Covers
// both the firstCommand offset and the clamping of commandCount.
void tst_QRhi::indirectCommandBufferSubrange()
{
    QFETCH(QRhi::Implementation, impl);
    QFETCH(QRhiInitParams *, initParams);
    ...
    std::unique_ptr<QRhiIndirectCommandBuffer> icb(rhi->newIndirectCommandBuffer(QRhiIndirectCommandBuffer::IndexedDraws, 4));
    QVERIFY(icb->create());

    icb->clear();
    icb->drawIndexed(6, 1, 0, 0, 0); // left red quad
    icb->drawIndexed(6, 1, 0, 4, 0); // right green quad
    icb->drawIndexed(6, 1, 0, 8, 0); // full width white quad, must not be drawn
    QCOMPARE(icb->commandCount(), 3u);
    u->commitIndirectCommandBuffer(icb.get());
    ...
    cb->executeIndirect(icb.get(), 1, 1);
    ...
    // Only the green quad, the left half stays at the blue clear color.
    checkIndirectCommandBufferResult(rhi, result, texture->pixelSize(), 0, 64, 64);
}

void tst_QRhi::indirectCommandBufferFromCompute_data()
{
    rhiTestData();
}

// The GPU-driven path: a compute shader writes the draw arguments, and
// buildIndirect() - called outside of any pass - populates the indirect command
// buffer from that buffer.
void tst_QRhi::indirectCommandBufferFromCompute()
{
    QFETCH(QRhi::Implementation, impl);
    QFETCH(QRhiInitParams *, initParams);
    ...
    ps->setVertexInputLayout(vlayout);
    ps->setShaderResourceBindings(graphicsSrb.get());
    ps->setRenderPassDescriptor(rpDesc.get());
    QVERIFY(ps->create());
    ...
    std::unique_ptr<QRhiIndirectCommandBuffer> icb(rhi->newIndirectCommandBuffer(QRhiIndirectCommandBuffer::IndexedDraws, 2));
    QVERIFY(icb->create());
    ...
    buildInfo.sourceBuffer = db.get();
    buildInfo.commandCount = 2;
    buildInfo.indexBuffer = ib.get();
    buildInfo.indexFormat = QRhiCommandBuffer::IndexUInt16;
    cb->buildIndirect(icb.get(), buildInfo);

    QVERIFY(icb->isGpuBuilt());
    QCOMPARE(icb->commandCount(), 2u);
    QCOMPARE(icb->recordedCommandCount(), 0u);
...
void tst_QRhi::indirectCommandBufferFromComputeSubrange_data()
{
    rhiTestData();
}

// Same as indirectCommandBufferFromCompute, but executing a subrange of the
// GPU-built commands: firstCommand and commandCount have to be honoured there
// just like with a CPU-recorded indirect command buffer.
void tst_QRhi::indirectCommandBufferFromComputeSubrange()
{
    QFETCH(QRhi::Implementation, impl);
    QFETCH(QRhiInitParams *, initParams);
    ...
    // Skip the first command, so only the right (green) quad is drawn
    cb->executeIndirect(icb.get(), 1, 1);
    ...
    // the left half stays the blue clear color
    checkIndirectCommandBufferResult(rhi, result, texture->pixelSize(), 0, 64, 64);
}

void tst_QRhi::indirectCommandBufferFromComputeCount_data()
{
    rhiTestData();
}

// The GPU-built path with a device-side count: the count buffer holds 1, so
// only the first of the two commands must be executed.
void tst_QRhi::indirectCommandBufferFromComputeCount()
{
    QFETCH(QRhi::Implementation, impl);
    QFETCH(QRhiInitParams *, initParams);
    ...
    if (!rhi->isFeatureSupported(QRhi::DrawIndirectCount))
        QSKIP("Indirect draw with a device-side count not supported on this backend");
    if (!rhi->isFeatureSupported(QRhi::BaseVertex))
        QSKIP("Base vertex not supported on this backend");
    ...
    static const quint32 DrawCount = 1;
    std::unique_ptr<QRhiBuffer> countBuf(rhi->newBuffer(QRhiBuffer::Static, QRhiBuffer::IndirectBuffer,
                                                        sizeof(quint32)));
    QVERIFY(countBuf->create());
    u->uploadStaticBuffer(countBuf.get(), &DrawCount);
    ...
    buildInfo.topology = ps->topology();
    buildInfo.sourceBuffer = db.get();
    buildInfo.commandCount = 2; // an upper bound now, the count buffer decides
    buildInfo.countBuffer = countBuf.get();
    buildInfo.indexBuffer = ib.get();
    buildInfo.indexFormat = QRhiCommandBuffer::IndexUInt16;
    cb->buildIndirect(icb.get(), buildInfo);
    ...
    // only the first command ran, so the right half stays the blue clear color
    checkIndirectCommandBufferResult(rhi, result, texture->pixelSize(), 64, 0, 64);
}

void tst_QRhi::indirectCommandBufferRerecord_data()
{
    rhiTestData();
}

// Records one set of commands, executes it, then clears and records a different
// set and executes that in a later frame. Covers re-encoding an indirect
// command buffer whose contents changed, which on Metal is keyed on the
// contents generation and on the other backends means re-uploading the buffer.
void tst_QRhi::indirectCommandBufferRerecord()
{
    QFETCH(QRhi::Implementation, impl);
    QFETCH(QRhiInitParams *, initParams);
    ...
    // Frame 0 draws both quads, frame 1 only the right one. Each frame gets its
    // own recording, so the second one has to actually take effect.
    for (int frame = 0; frame < 2; ++frame) {
        ...
        if (frame == 0)
            icb->drawIndexed(6, 1, 0, 0, 0); // left red quad
        icb->drawIndexed(6, 1, 0, 4, 0);     // right green quad
        QCOMPARE(icb->commandCount(), frame == 0 ? 2u : 1u);
        u->commitIndirectCommandBuffer(icb.get());
        ...
        if (frame == 0)
            checkIndirectCommandBufferResult(rhi, result, texture->pixelSize(), 64, 64, 0);
        else // the left half is back to the blue clear color
            checkIndirectCommandBufferResult(rhi, result, texture->pixelSize(), 0, 64, 64);
    }
}

void tst_QRhi::indirectCommandBufferCpuThenBuild_data()
{
    rhiTestData();
}

// Uses the same indirect command buffer first with CPU-recorded commands and
// then with buildIndirect(). Backends that keep different native objects for
// the two - Metal allocates its MTLIndirectCommandBuffers with a different
// storage mode depending on who fills them - have to switch over cleanly.
void tst_QRhi::indirectCommandBufferCpuThenBuild()
{
    QFETCH(QRhi::Implementation, impl);
    QFETCH(QRhiInitParams *, initParams);
    ...
    // Frame 0: CPU-recorded, only the right quad. Frame 1: the same object
    // rebuilt from the compute-written buffer, which draws both.
    for (int frame = 0; frame < 2; ++frame) {
        QRhiCommandBuffer *cb = nullptr;
        QVERIFY(rhi->beginOffscreenFrame(&cb) == QRhi::FrameOpSuccess);
        QVERIFY(cb);
        ...
        if (frame == 0) {
            ...
            buildInfo.topology = ps->topology();
            buildInfo.sourceBuffer = db.get();
            // commandCount left at 0, which means maxCommandCount(), so 2
            buildInfo.indexBuffer = ib.get();
            buildInfo.indexFormat = QRhiCommandBuffer::IndexUInt16;
            cb->buildIndirect(icb.get(), buildInfo);
            ...
            QVERIFY(icb->isGpuBuilt());
            QCOMPARE(icb->commandCount(), 2u);
            // the CPU-side recording from frame 0 is still there, but ignored
            QCOMPARE(icb->recordedCommandCount(), 1u);
        }
        ...
        if (frame == 0) // only the right quad, left half stays blue
            checkIndirectCommandBufferResult(rhi, result, texture->pixelSize(), 0, 64, 64);
        else
            checkIndirectCommandBufferResult(rhi, result, texture->pixelSize(), 64, 64, 0);
    }
...
void tst_QRhi::indexedIndirectMultiDrawHighDrawCount_data()
{
    rhiTestData();
}

// Verifies that indirect drawing works correctly with a high draw count
// (above the ICB_DRAW_COUNT_THRESHOLD of 128). On Metal, when
// UsesIndirectDraws is set, draw counts above 128 trigger the GPU-driven
// Indirect Command Buffer (ICB) path. This test ensures that path produces
// the same rendering result as the CPU-side loop fallback.
void tst_QRhi::indexedIndirectMultiDrawHighDrawCount()
{
    QFETCH(QRhi::Implementation, impl);
    QFETCH(QRhiInitParams *, initParams);
    ...
    // 130 draws: above the Metal ICB threshold of 128. The first command
    // draws the left (red) quad, the second draws the right (green) quad,
    // and the remaining 128 re-draw the left quad on top (red over red).
    // The net visible result is identical to the 2-draw baseline.
    static constexpr int DrawCount = 130;
    QRhiIndexedIndirectDrawCommand cmds[DrawCount];
    cmds[0] = { 6, 1, 0, 0, 0 }; // left quad (red)
    cmds[1] = { 6, 1, 0, 4, 0 }; // right quad (green)
    for (int i = 2; i < DrawCount; ++i)
        cmds[i] = { 6, 1, 0, 0, 0 }; // left quad again (red over red)
    ...
    std::unique_ptr<QRhiBuffer> vb(
        rhi->newBuffer(QRhiBuffer::Immutable,
                       QRhiBuffer::VertexBuffer,
                       sizeof(IndirectTestVertices)));
    QVERIFY(vb->create());
    u->uploadStaticBuffer(vb.get(), IndirectTestVertices);

    std::unique_ptr<QRhiBuffer> ib(
        rhi->newBuffer(QRhiBuffer::Immutable,
                       QRhiBuffer::IndexBuffer,
                       sizeof(IndirectTestIndices)));
    QVERIFY(ib->create());
    u->uploadStaticBuffer(ib.get(), IndirectTestIndices);
    ...
    srb->setBindings({
        QRhiShaderResourceBinding::uniformBuffer(
            0, QRhiShaderResourceBinding::VertexStage, ubuf.get())
    });
    QVERIFY(srb->create());
    ...
    vlayout.setAttributes({
        { 0, 0, QRhiVertexInputAttribute::Float3,
          offsetof(IndirectTestVertex, x) },
        { 0, 1, QRhiVertexInputAttribute::Float4,
          offsetof(IndirectTestVertex, r) },
    });
    ps->setVertexInputLayout(vlayout);
    ps->setFlags(QRhiGraphicsPipeline::UsesIndirectDraws);
    ps->setShaderResourceBindings(srb.get());
    ...
    cb->drawIndexedIndirect(db.get(), 0, DrawCount);

    QRhiReadbackResult rb;
    QImage result;
    rb.completed = [&] {
        result = QImage(
            reinterpret_cast<const uchar *>(rb.data.constData()),
            rb.pixelSize.width(), rb.pixelSize.height(),
            QImage::Format_RGBA8888_Premultiplied);
    };
    QRhiResourceUpdateBatch *u2 = rhi->nextResourceUpdateBatch();
    ...
    const int y = result.height() / 2; // Check a horizontal line in the middle
    const quint32 *p =
        reinterpret_cast<const quint32 *>(result.constScanLine(y));
    ...
    for (int x = 0; x < result.width(); ++x) {
        const QRgb c(p[x]);
        if (qRed(c) >= (255 - maxFuzz)
                && qGreen(c) == 0 && qBlue(c) == 0)
            ++redCount;
        else if (qRed(c) == 0
                 && qGreen(c) >= (255 - maxFuzz) && qBlue(c) == 0)
            ++greenCount;
        else if (qRed(c) == 0
                 && qGreen(c) == 0 && qBlue(c) >= (255 - maxFuzz))
            ++blueCount;
        else
            QFAIL(qPrintable(
                QStringLiteral("Unexpected color at x=%1 y=%2: "
                               "%3,%4,%5")
                    .arg(x).arg(y)
                    .arg(qRed(c)).arg(qGreen(c)).arg(qBlue(c))));
    }
...
// Like indexedIndirectMultiDrawHighDrawCount, but with a viewport covering
// only the left half of the render target. On Metal the ICB path interrupts
// and restarts the render pass, which must not lose the viewport.
void tst_QRhi::indexedIndirectMultiDrawHighDrawCountViewport()
{
    QFETCH(QRhi::Implementation, impl);
    QFETCH(QRhiInitParams *, initParams);
    ...
    static constexpr int DrawCount = 130; // above the Metal ICB threshold of 128
    QRhiIndexedIndirectDrawCommand cmds[DrawCount];
    cmds[0] = { 6, 1, 0, 0, 0 }; // left quad (red)
    cmds[1] = { 6, 1, 0, 4, 0 }; // right quad (green)
    for (int i = 2; i < DrawCount; ++i)
    ...
    cb->setShaderResources(srb.get());
    // Left half only.
    cb->setViewport({ 0, 0, float(outputSize.width() / 2), float(outputSize.height()) });
    ...
    // Redraws the left quad, so it does not change the result, but recording a
    // draw call after the pass was interrupted has to work.
    cb->drawIndexed(6);
    ...
    // Both quads squeezed into the left half, the right half left at the clear color.
    QCOMPARE(redCount, 32);
    QCOMPARE(greenCount, 32);
    QCOMPARE(blueCount, 64);
}

void tst_QRhi::indexedIndirectMultiDrawHighDrawCountMultisample_data()
{
    rhiTestData();
}

// Draws into a multisample render target, first with a normal draw call and then
// with a high draw count indirect draw. On Metal the latter takes the ICB path,
// which interrupts and restarts the render pass; that must not discard what the
// first draw put into the multisample attachment.
void tst_QRhi::indexedIndirectMultiDrawHighDrawCountMultisample()
{
    QFETCH(QRhi::Implementation, impl);
    QFETCH(QRhiInitParams *, initParams);
    ...
    if (!rhi->isFeatureSupported(QRhi::MultisampleRenderBuffer))
        QSKIP("Multisample renderbuffer not supported on this backend");

    const int sampleCount = 4;
    static constexpr int DrawCount = 130; // above the Metal ICB threshold of 128
    QRhiIndexedIndirectDrawCommand cmds[DrawCount];
    for (int i = 0; i < DrawCount; ++i)
        cmds[i] = { 6, 1, 0, 4, 0 }; // right quad (green)

    const QSize outputSize(128, 64);
    std::unique_ptr<QRhiRenderBuffer> msaaBuf(rhi->newRenderBuffer(QRhiRenderBuffer::Color, outputSize, sampleCount));
    QVERIFY(msaaBuf->create());

    std::unique_ptr<QRhiTexture> resolveTex(
                rhi->newTexture(QRhiTexture::RGBA8, outputSize, 1,
                                QRhiTexture::RenderTarget | QRhiTexture::UsedAsTransferSource));
    QVERIFY(resolveTex->create());

    QRhiColorAttachment colorAtt(msaaBuf.get());
    colorAtt.setResolveTexture(resolveTex.get());
    std::unique_ptr<QRhiTextureRenderTarget> rt(rhi->newTextureRenderTarget({ colorAtt }));
    std::unique_ptr<QRhiRenderPassDescriptor> rpDesc(rt->newCompatibleRenderPassDescriptor());
    rt->setRenderPassDescriptor(rpDesc.get());
    QVERIFY(rt->create());
    ...
    // Left (red) quad with a normal draw call, before any pass interruption.
    cb->drawIndexed(6);

    // Right (green) quad, with enough draws to trigger the ICB path on Metal.
    cb->drawIndexedIndirect(db.get(), 0, DrawCount);
    ...
    QRhiResourceUpdateBatch *u2 = rhi->nextResourceUpdateBatch();
    u2->readBackTexture({ resolveTex.get() }, &rb);
    cb->endPass(u2);
    ...
    result.convertTo(QImage::Format_ARGB32);
    QCOMPARE(result.size(), resolveTex->pixelSize());

    const int y = result.height() / 2;
    const quint32 *p = reinterpret_cast<const quint32 *>(result.constScanLine(y));

    int redCount = 0, greenCount = 0, otherCount = 0;
    const int maxFuzz = 1;
    for (int x = 0; x < result.width(); ++x) {
        const QRgb c(p[x]);
        if (qRed(c) >= (255 - maxFuzz) && qGreen(c) == 0 && qBlue(c) == 0)
    ...
    // The red quad was drawn before the interruption, the green one after it.
    QCOMPARE(redCount, 64);
    QCOMPARE(greenCount, 64);
    QCOMPARE(otherCount, 0);
}

void tst_QRhi::indexedIndirectMultiDrawHighDrawCountMultisampleDepthResolve_data()
{
    rhiTestData();
}

// Same idea as indexedIndirectMultiDrawHighDrawCountMultisample, but the
// interruption has to preserve the multisample depth buffer, not the color one.
// A near quad is drawn normally, then a farther, full width quad with a high
// draw count indirect draw. The near quad has to win the depth test, which it
// can only do if the depth contents survived the interruption. The depth
// attachment has a resolve texture, so its store action only resolves.
void tst_QRhi::indexedIndirectMultiDrawHighDrawCountMultisampleDepthResolve()
{
    QFETCH(QRhi::Implementation, impl);
    QFETCH(QRhiInitParams *, initParams);
    ...
    if (!rhi->isFeatureSupported(QRhi::MultisampleTexture))
        QSKIP("Multisample texture not supported on this backend");
    if (!rhi->isFeatureSupported(QRhi::ResolveDepthStencil))
        QSKIP("Depth-stencil resolve not supported on this backend");

    const int sampleCount = 4;
    // Near quad on the left, then a farther quad covering everything.
    static constexpr IndirectTestVertex Vertices[] = {
        { -1.f, -1.f, -0.5f,  1.f, 0.f, 0.f, 1.f },
        {  0.f, -1.f, -0.5f,  1.f, 0.f, 0.f, 1.f },
        {  0.f,  1.f, -0.5f,  1.f, 0.f, 0.f, 1.f },
    ...
    QRhiIndexedIndirectDrawCommand cmds[DrawCount];
    for (int i = 0; i < DrawCount; ++i)
        cmds[i] = { 6, 1, 0, 4, 0 }; // the far quad

    const QSize outputSize(128, 64);
    std::unique_ptr<QRhiTexture> msaaTex(
                rhi->newTexture(QRhiTexture::RGBA8, outputSize, sampleCount,
                                QRhiTexture::RenderTarget));
    QVERIFY(msaaTex->create());
    std::unique_ptr<QRhiTexture> resolveTex(
                rhi->newTexture(QRhiTexture::RGBA8, outputSize, 1,
                                QRhiTexture::RenderTarget | QRhiTexture::UsedAsTransferSource));
    QVERIFY(resolveTex->create());

    std::unique_ptr<QRhiTexture> msaaDepthTex(
                rhi->newTexture(QRhiTexture::D32F, outputSize, sampleCount,
                                QRhiTexture::RenderTarget));
    QVERIFY(msaaDepthTex->create());
    std::unique_ptr<QRhiTexture> depthResolveTex(
                rhi->newTexture(QRhiTexture::D32F, outputSize, 1, QRhiTexture::RenderTarget));
    QVERIFY(depthResolveTex->create());

    QRhiColorAttachment colorAtt(msaaTex.get());
    colorAtt.setResolveTexture(resolveTex.get());
    QRhiTextureRenderTargetDescription rtDesc(colorAtt);
    rtDesc.setDepthTexture(msaaDepthTex.get());
    rtDesc.setDepthResolveTexture(depthResolveTex.get());
    ...
    // Near quad on the left, writing depth, before the interruption.
    cb->drawIndexed(6);

    // Farther, full width quad. Must be rejected where the near quad already is.
    cb->drawIndexedIndirect(db.get(), 0, DrawCount);
    ...
    // Left half kept by the near quad, right half taken by the farther one.
    QCOMPARE(redCount, 64);
    QCOMPARE(greenCount, 64);
    QCOMPARE(otherCount, 0);
}

void tst_QRhi::indexedIndirectMultiDrawHighDrawCountRenderBufferDepth_data()
{
    rhiTestData();
}

// Depth testing across a pass interruption with a QRhiRenderBuffer as the
// depth-stencil buffer, which is what QRhiSwapChain requires. Such a
// renderbuffer may be backed by transient memory, in which case its contents
// cannot survive the interruption; QRhiRenderBuffer::NoTransientBacking opts out
// of that. A near quad is drawn normally, then a farther, full width quad with a
// high draw count indirect draw, and the near one has to win the depth test.
void tst_QRhi::indexedIndirectMultiDrawHighDrawCountRenderBufferDepth()
{
    QFETCH(QRhi::Implementation, impl);
    QFETCH(QRhiInitParams *, initParams);
...
void tst_QRhi::indexedIndirectDrawCountMultisample_data()
{
    rhiTestData();
}

// drawIndexedIndirectCount() into a multisample render target. On Metal this
// always interrupts the render pass, so the normal draw preceding it must
// survive, just like in indexedIndirectMultiDrawHighDrawCountMultisample.
void tst_QRhi::indexedIndirectDrawCountMultisample()
{
    QFETCH(QRhi::Implementation, impl);
    QFETCH(QRhiInitParams *, initParams);
    ...
    const int sampleCount = 4;
    // Two commands, but only the first one may be issued.
    static constexpr QRhiIndexedIndirectDrawCommand IndirectDrawCommands[] = {
        { 6, 1, 0, 4, 0 }, // right quad (green)
        { 6, 1, 0, 0, 0 }, // left quad (red), must not overdraw
    };
    static constexpr quint32 DrawCount = 1;
    ...
    std::unique_ptr<QRhiBuffer> argsBuf(
        rhi->newBuffer(QRhiBuffer::Immutable, QRhiBuffer::IndirectBuffer,
                       sizeof(IndirectDrawCommands)));
    QVERIFY(argsBuf->create());
    u->uploadStaticBuffer(argsBuf.get(), IndirectDrawCommands);

    std::unique_ptr<QRhiBuffer> countBuf(
        rhi->newBuffer(QRhiBuffer::Immutable, QRhiBuffer::IndirectBuffer, sizeof(quint32)));
    QVERIFY(countBuf->create());
    u->uploadStaticBuffer(countBuf.get(), &DrawCount);
    ...
    // Left (red) quad with a normal draw call, before the pass interruption.
    cb->drawIndexed(6);

    // Right (green) quad from the first indirect command only.
    cb->drawIndexedIndirectCount(argsBuf.get(), 0, countBuf.get(), 0,
                                 quint32(std::size(IndirectDrawCommands)));
...
void tst_QRhi::oneDimTexture()
{
    ...
    // present and enabled, according to the validation layer. Yet it seems to
    // work, and enabling that device extension seem to make no difference when
    // it comes to the validation layer warnings, which is odd. In any case,
    // skip these on Vulkan for now.
    if (impl != QRhi::Vulkan) {
