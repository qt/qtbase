...
function (qt_internal_setup_wasm_target_properties wasmTarget)
    ...
    if (QT_FEATURE_sanitize_address OR QT_FEATURE_sanitize_undefined)
        ...
        target_link_options("${wasmTarget}" INTERFACE "SHELL:-O0")
        target_link_options("${wasmTarget}" INTERFACE "SHELL:-g3")
        target_link_options("${wasmTarget}" INTERFACE "SHELL:-sALLOW_MEMORY_GROWTH=1")
        target_link_options("${wasmTarget}" INTERFACE "SHELL:-fsanitize-recover=all")
        if (QT_FEATURE_sanitize_address)
            target_link_options("${wasmTarget}" INTERFACE "SHELL:-fsanitize=null")
            target_link_options("${wasmTarget}" INTERFACE "SHELL:-fsanitize-address-use-after-scope")
        endif()
        ...
        target_compile_options("${wasmTarget}" INTERFACE "SHELL:-g3")
        target_compile_options("${wasmTarget}" INTERFACE "SHELL:-fno-inline")
        target_compile_options("${wasmTarget}" INTERFACE "SHELL:-fno-omit-frame-pointer")
        target_compile_options("${wasmTarget}" INTERFACE "SHELL:-fsanitize-recover=all")
        target_compile_options("${wasmTarget}" INTERFACE "SHELL:-fsanitize=null")
        if (QT_FEATURE_sanitize_address)
            target_compile_options("${wasmTarget}" INTERFACE "SHELL:-fsanitize=null")
            target_compile_options("${wasmTarget}" INTERFACE "SHELL:-fsanitize-address-use-after-scope")
        endif()
    endif()
