...
{
  if (!m_copyrightText.isEmpty())
    return m_copyrightText.toUtf8();
...
{
  if (!m_copyrightText.isEmpty() || !emit_qt_code)
    return QString();
...
#undef GOTO
  ...
  { // decls...
    ...
    out << startIncludeGuard(declFileName) << Qt::endl;
    ...
    out << endIncludeGuard(declFileName) << Qt::endl;
  } // end decls
