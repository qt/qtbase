...
/*!
    ...
    \note When writing a new record to the database, an \c autoValue field will
    only hold the new value generated by the database once the record has been
    submitted to the database. When using the ODBC driver, due to limitations
    in the ODBC API, the \c isAutoValue() field is only populated in a
    QSqlField resulting from a QSqlRecord obtained by executing a \c SELECT
    query. It is \c false for a QSqlField resulting from a QSqlRecord returned
    from QSqlDatabase::record() or QSqlDatabase::primaryIndex().
*/
