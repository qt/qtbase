...
/*!
    ...
    The Vulkan implementation, meaning the Vulkan loader, the drivers, and any
    enabled layers, is a trusted, in-process platform dependency. Qt loads it
    and calls into it directly, without any sandboxing or verification, in the
    same way as it treats the OpenGL implementation.

    All data consumed by QVulkanInstance and the classes building on it, such as
    QVulkanWindow and QRhi with its Vulkan backend, is expected to be trusted
    content. This includes the layer and extension names passed to setLayers()
    and setExtensions(), a native \c VkInstance handle adopted via
    setVkInstance(), and all rendering content and Vulkan command parameters
