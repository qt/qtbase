...
/*!
  ...
  All data consumed by QVulkanWindow is expected to be trusted content. This
  includes the device extension names passed to setDeviceExtensions(), the
  device creation parameters written by the callbacks registered via
  setEnabledFeaturesModifier() and setQueueCreateInfoModifier(), and all
  rendering content and Vulkan command parameters generated by the
  QVulkanWindowRenderer implementation. The Vulkan implementation itself is a
  trusted, in-process platform dependency, see
  \l{QVulkanInstance#Security Considerations}{QVulkanInstance} for details.
  ...
  that is not part of the application and is not under the developers' control.
