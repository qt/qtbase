...
/*!
    ...
    \value [since 6.13] DispatchIndirect Indicates that the
    \l{QRhiCommandBuffer::dispatchIndirect()}{dispatchIndirect()} function is
    available, allowing compute work group counts to be sourced from a
...
/*!
    ...
    \value [since 6.12] IndirectBuffer Indirect draw or dispatch buffer. This
    allows the QRhiBuffer to be used in
    \l{QRhiCommandBuffer::drawIndirect()}{drawIndirect()},
    \l{QRhiCommandBuffer::drawIndexedIndirect()}{drawIndexedIndirect()}, and
    \l{QRhiCommandBuffer::dispatchIndirect()}{dispatchIndirect()}.
    ...
    \l{QRhi::Compute}{compute shaders}, allowing indirect draw or dispatch
    commands to be generated by compute shaders and consumed by indirect
    draw/dispatch calls.
 */
...
/*!
    ...
    A compute dispatch command that can be uploaded to a QRhiBuffer of usage
    QRhiBuffer::UsageFlag::IndirectBuffer. The fields specify the number of
    local work groups along each dimension and have the same meaning as the
    \c x, \c y, \c z parameters of QRhiCommandBuffer::dispatch().

    \c y and \c z default to 1, so a one dimensional dispatch only needs \c x
    to be set. Note that a count of 0 in any dimension is valid and results in
    no work groups being launched at all.

    \sa QRhiCommandBuffer::dispatchIndirect()
...
/*!
    ...
    \since 6.13
 */
void QRhiCommandBuffer::dispatchIndirect(QRhiBuffer *indirectBuffer,
                                         quint32 indirectBufferOffset)
{
    Q_ASSERT(indirectBuffer);
    Q_ASSERT(indirectBuffer->usage().testFlag(QRhiBuffer::IndirectBuffer));
    Q_ASSERT_X((indirectBufferOffset & 3u) == 0u, Q_FUNC_INFO, "indirectBufferOffset must be a multiple of 4");
    m_rhi->dispatchIndirect(this, indirectBuffer, indirectBufferOffset);
}
