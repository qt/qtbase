...
{
    return entityStr < entity.name;
}

static bool operator<(const QTextHtmlEntity &entity, QStringView entityStr)
{
    return entity.name < entityStr;
}

static QChar resolveEntity(QStringView entity)
{
    const QTextHtmlEntity *end = std::end(entities);
    const QTextHtmlEntity *e = std::lower_bound(std::begin(entities), end, entity);
    if (e == end || (entity < *e))
        return QChar();
    return e->code;
}
