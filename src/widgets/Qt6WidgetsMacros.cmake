...
function(qt6_add_ui target)
    ...
    foreach(source_file ${sources})
        ...
        _qt_internal_path_is_prefix(CMAKE_SOURCE_DIR "${infile}" is_in_source_dir)
        _qt_internal_path_is_prefix(CMAKE_BINARY_DIR "${infile}" is_in_binary_dir)
        if (is_in_source_dir)
            string(REPLACE "${CMAKE_SOURCE_DIR}" "${target}/src_dir" infile_to_hash "${infile}")
        elseif(is_in_binary_dir)
            string(REPLACE "${CMAKE_BINARY_DIR}" "${target}/build_dir" infile_to_hash "${infile}")
        else()
            file(RELATIVE_PATH infile_to_hash "${CMAKE_SOURCE_DIR}" "${infile}")
            set(infile_to_hash "${target}/ext_dir/${infile_to_hash}")
        endif()
