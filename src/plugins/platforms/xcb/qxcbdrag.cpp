...
QVariant QXcbDropData::xdndObtainData(const QByteArray &format, QMetaType requestedType) const
{
...
#ifndef QT_NO_CLIPBOARD
    if (c->clipboard()->getSelectionOwner(drag->atom(QXcbAtom::AtomXdndSelection)) == XCB_NONE)
    ...
    if (!result.has_value())
        return QVariant();
    return mimeConvertToFormat(c, a, result.value(), QLatin1String(format), requestedType, hasUtf8);
#else
    return QVariant();
#endif
}
