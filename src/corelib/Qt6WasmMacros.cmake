...
function(_qt_internal_wasm_add_target_helpers target)
    ...
    if("${targetType}" STREQUAL "EXECUTABLE")
        ...
        # Shared builds preload the Qt plugin and qml manifests that wasmdeployqt
        # generates. Apps add their own manifests through qt_add_wasm_preload,
        # which are collected here.
        set(_wasm_preload_files "")
        if(QT_FEATURE_shared)
            list(APPEND _wasm_preload_files "qt_plugins.json" "qt_qml_imports.json")
        endif()

        get_target_property(_app_preload_manifest "${target}" _qt_wasm_app_preload_manifest)
        if(_app_preload_manifest)
            list(APPEND _wasm_preload_files "${_app_preload_manifest}")
        endif()

        if(_wasm_preload_files)
            set(_preload_js "")
            foreach(_f IN LISTS _wasm_preload_files)
                string(APPEND _preload_js "'${_f}', ")
            endforeach()
            set(PRELOAD "preload: [${_preload_js}],")
        else()
            set(PRELOAD "")
        endif()
        ...
        # Convenience deploy target for shared MAIN_MODULE apps, so that
        # cmake --build . --target <app>_deploy produces a runnable directory.
        if(QT_FEATURE_shared AND NOT is_test AND QT_HOST_PATH)
            if(target_output_directory)
                set(_deploy_workdir "${target_output_directory}")
            else()
                set(_deploy_workdir "${CMAKE_CURRENT_BINARY_DIR}")
            endif()
            add_custom_target("${target}_deploy"
                COMMAND "${QT_HOST_PATH}/bin/wasmdeployqt"
                    "$<TARGET_FILE_DIR:${target}>/$<TARGET_FILE_BASE_NAME:${target}>.wasm"
                    "--qt-wasm-dir=${WASM_BUILD_DIR}/bin"
                    "--qt-host-dir=${QT_HOST_PATH}/bin"
                WORKING_DIRECTORY "${_deploy_workdir}"
                DEPENDS "${target}"
                VERBATIM)
        endif()
        ...
        elseif(NOT DEFINED QT_WASM_INITIAL_MEMORY)
            if (QT_FEATURE_sanitize_address OR QT_FEATURE_sanitize_undefined)
                set(QT_WASM_INITIAL_MEMORY "200MB")
            else()
...
function(_qt_internal_add_wasm_extra_exported_methods target)
    ...
    set(wasm_default_exported_methods "UTF16ToString,stringToUTF16,JSEvents,specialHTMLTargets,FS,SOCKFS,callMain")
...
# Declare files or side module targets to preload into the app's in-memory
# filesystem at startup. Each item is copied next to the app at build time and
# a manifest is generated and added to the app's qtLoad preload list.
# Preloading is one way to make app private plugins and other data that
# wasmdeployqt does not handle, available to the app, as an alternative to
# loading them on demand at runtime.
function(qt_add_wasm_preload target)
    cmake_parse_arguments(arg "" "DESTINATION" "FILES;TARGETS" ${ARGN})
    if(NOT arg_DESTINATION)
        message(FATAL_ERROR "qt_add_wasm_preload requires a DESTINATION directory.")
    endif()
    ...
    # Plain files are copied verbatim next to the app and preloaded from there.
    foreach(_file IN LISTS arg_FILES)
        get_filename_component(_base "${_file}" NAME)
        add_custom_command(TARGET "${target}" POST_BUILD
            COMMAND "${CMAKE_COMMAND}" -E copy_if_different "${_file}" "${_out_dir}/${_base}")
        string(APPEND _entries
            "    { \"source\": \"${_base}\", \"destination\": \"${arg_DESTINATION}/${_base}\" },\n")
    endforeach()

    # Side module targets are copied by their built file name.
    foreach(_tgt IN LISTS arg_TARGETS)
        add_custom_command(TARGET "${target}" POST_BUILD
            COMMAND "${CMAKE_COMMAND}" -E copy_if_different
                "$<TARGET_FILE:${_tgt}>" "${_out_dir}/$<TARGET_FILE_NAME:${_tgt}>")
        string(APPEND _entries
            "    { \"source\": \"$<TARGET_FILE_NAME:${_tgt}>\", \"destination\": \"${arg_DESTINATION}/$<TARGET_FILE_NAME:${_tgt}>\" },\n")
    endforeach()

    # Drop the trailing comma so the generated array is valid JSON.
    string(REGEX REPLACE ",\n$" "\n" _entries "${_entries}")

    set(_manifest "${_out_name}_preload.json")
    file(GENERATE OUTPUT "${_out_dir}/${_manifest}" CONTENT "[\n${_entries}]\n")

    set_target_properties("${target}" PROPERTIES _qt_wasm_app_preload_manifest "${_manifest}")
endfunction()
