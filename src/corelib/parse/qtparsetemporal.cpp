                                    ...
                                    TemporalFieldFlags flags)
{
    ...
    const auto beginLength = [flex = flags.testFlag(F::FlexSpace),
                              strict = flags.testFlag(F::StrictSpace)]
        ...
        const auto spacingForward = [](QStringIterator &iter) {
        ...
        const auto matchingSpace = [](char32_t need, char32_t have) {
            constexpr char32_t ASCIImax = 0x7f;
            // Treat SPACE and non-ASCII spaces as equivalent:
            if (need == U' ' || need > ASCIImax)
                return have == U' ' || have > ASCIImax;
        ...
        if (flex || !strict) {
            ...
            // Advance through the string; on each space, check match since last
            // space then matching of spaces; and prune view, target past match.
            while (iter.hasNext()) {
                ...
                if (const char32_t tgt = iter.next(); QChar::isSpace(tgt)) {
                    ...
                    if (const char32_t got = viter.next(); !QChar::isSpace(got))
                        ...
                        same = spacingForward(viter);
                    else if (!matchingSpace(tgt, got))
                        return failed;
                    else
                        same = viter.index();
                    matched += same;
            ...
            // the same way we would the whole would be in strict mode.
        }
                                    ...
                                    TemporalFieldFlags flags, FieldConfig &&config) const
{
    ...
    for (; digits.size() >= width; digits.chop(1)) {
        ...
        auto optvalue = [whole, forbidZero = !config.allowZero,
                                    ...
                                    const TemporalField &field) const
{
    ...
    case Cat::SecondFraction:
        ...
        matches = numericExtend(base, text, field.options,
                                {millisTarget, base.results.millis >= 0, 999, true, field.width,
                                 field.options.testFlag(Flag::RoundFraction) ? 0 : 3, 3});
        break;
    case Cat::Second:
        matches = numericExtend(base, text, field.options,
                                {secondTarget, base.results.second >= 0, 59, true, field.width, 2});
        break;
        // case Cat::MinuteFraction: break;
    case Cat::Minute:
        matches = numericExtend(base, text, field.options,
                                {minuteTarget, base.results.minute >= 0, 59, true, field.width, 2});
        break;
        // case Cat::HourFraction: break;
    case Cat::HourMod12:
        matches = numericExtend(base, text, field.options,
                                {hourMod12Target, base.hourMod12 > 0, 12, false, field.width, 2});
        break;
    case Cat::Hour:
        matches = numericExtend(base, text, field.options,
                                {hourTarget, base.results.hour >= 0, 23, true, field.width, 2});
        break;
    ...
    case Cat::DayOfMonth: {
        ...
        matches = numericExtend(base, text, field.options,
                                {dayOfMonthTarget, base.results.dayOfMonth > 0, maxDays, false,
        ...
        if (matchesFlagWithin(field.options, Flag::Numeric, FieldGroup::FormMask)) {
            auto extend = numericExtend(base, text, field.options,
                                        {monthTarget, base.results.month > 0,
                                         calendar.maximumMonthsInYear(), false, field.width, 2});
            if (matches.empty())
