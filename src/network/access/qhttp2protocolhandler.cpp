                                ...
                                bool useProxy)
{
    ...
    for (qsizetype i = 0; i < requestHeader.size(); ++i) {
        ...
        if (name == QHttpHeaders::wellKnownHeaderName(WK::Connection)
            || name == QHttpHeaders::wellKnownHeaderName(WK::Host)
            || name == QHttpHeaders::wellKnownHeaderName(WK::KeepAlive)
            || name.compare("proxy-connection"_L1, Qt::CaseInsensitive) == 0
            || name == QHttpHeaders::wellKnownHeaderName(WK::TransferEncoding)) {
        ...
        header.emplace_back(QByteArray{name.data(), name.size()}.toLower(),
